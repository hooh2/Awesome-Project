...
void CarSetting::init()
{
	auto scene = aiImportFile("models/Cars_FV.OBJ", aiProcessPreset_TargetRealtime_MaxQuality);
...
void CarSetting::update()
{
	...
	car.setCarColisionRespondCars(collisionRespond);

	this->entity->setCarObject(car);
}

void CarSetting::calcBoundingBox(std::vector<std::vector<glm::vec3>> triangles, std::vector<glm::vec3>& boundingBox)
{
...
bool CarSetting::inBoundingBox(glm::vec3 point, std::vector<glm::vec3> boundingBox)
{
...
bool CarSetting::collisionCheck(glm::vec3 collisionContourPoint)
{
...
bool CarSetting::collisionBetweenPoint(glm::vec3 point, float threshold, unsigned int triangle)
{
...
bool CarSetting::BarycentricCalculation(glm::vec3 point, float dist, unsigned int triangle)
{
