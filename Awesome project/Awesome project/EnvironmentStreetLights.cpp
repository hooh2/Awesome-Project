...
EnvironmentStreetLights::EnvironmentStreetLights() {}
EnvironmentStreetLights::~EnvironmentStreetLights() {}

void EnvironmentStreetLights::init()
{
	auto scene = aiImportFile("models/Environment_street_lights.OBJ", aiProcessPreset_TargetRealtime_MaxQuality);
	...
	if (scene && scene->mNumMeshes != 0)
	{
		...
		{
			printf("Loading Environment (Street Lights) Textures ... ");
...
void EnvironmentStreetLights::update()
{
	this->entity->getCarObject(car);
	...
	car.setCarCollisionRespondEnvStreetLights(collisionRespond);

	this->entity->setCarObject(car);
}

void EnvironmentStreetLights::calcBoundingBox(std::vector<std::vector<glm::vec3>> triangles, std::vector<glm::vec3>& boundingBox)
{
...
bool EnvironmentStreetLights::inBoundingBox(glm::vec3 point, std::vector<glm::vec3> boundingBox)
{
...
bool EnvironmentStreetLights::collisionCheck(glm::vec3 collisionContourPoint)
{
	for (unsigned int b = 0; b < modelBBs.size(); b++)
	{
		if (inBoundingBox(collisionContourPoint, modelBBs[b]))
		{
			...
			{
				if (collisionBetweenPoint(collisionContourPoint, 3.f, tiangles_in_BBs[b][t]))
				{
					return true;
				}
			}
...
bool EnvironmentStreetLights::collisionBetweenPoint(glm::vec3 point, float threshold, unsigned int triangle)
{
...
bool EnvironmentStreetLights::BarycentricCalculation(glm::vec3 point, float dist, unsigned int triangle)
{
