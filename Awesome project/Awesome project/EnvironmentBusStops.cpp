...
EnvironmentBusStops::EnvironmentBusStops() {}
EnvironmentBusStops::~EnvironmentBusStops() {}

void EnvironmentBusStops::init()
{
	auto scene = aiImportFile("models/Environment_bus_stops.OBJ", aiProcessPreset_TargetRealtime_MaxQuality);
	...
	if (scene && scene->mNumMeshes != 0)
	{
		...
		{
			printf("Loading Environment (Bus Stops) Textures ... ");
...
void EnvironmentBusStops::update()
{
	this->entity->getCarObject(car);
	...
	car.setCarCollisionRespondEnvBusStops(collisionRespond);

	this->entity->setCarObject(car);
}

void EnvironmentBusStops::calcBoundingBox(std::vector<std::vector<glm::vec3>> triangles, std::vector<glm::vec3>& boundingBox)
{
...
bool EnvironmentBusStops::inBoundingBox(glm::vec3 point, std::vector<glm::vec3> boundingBox)
{
...
bool EnvironmentBusStops::collisionCheck(glm::vec3 collisionContourPoint)
{
...
bool EnvironmentBusStops::collisionBetweenPoint(glm::vec3 point, float threshold, unsigned int triangle)
{
...
bool EnvironmentBusStops::BarycentricCalculation(glm::vec3 point, float dist, unsigned int triangle)
{
