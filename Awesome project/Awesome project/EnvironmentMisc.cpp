...
EnvironmentMisc::EnvironmentMisc() {}
EnvironmentMisc::~EnvironmentMisc() {}

void EnvironmentMisc::init()
{
	auto scene = aiImportFile("models/Environment_misc.OBJ", aiProcessPreset_TargetRealtime_MaxQuality);
	...
	if (scene && scene->mNumMeshes != 0)
	{
		...
		{
			printf("Loading Environment (Misc) Textures ... ");
...
void EnvironmentMisc::update()
{
	...
	car.setCarCollisionRespondEnvMisc(collisionRespond);

	this->entity->setCarObject(car);
}

void EnvironmentMisc::calcBoundingBox(std::vector<std::vector<glm::vec3>> triangles, std::vector<glm::vec3>& boundingBox)
{
...
bool EnvironmentMisc::inBoundingBox(glm::vec3 point, std::vector<glm::vec3> boundingBox)
{
...
bool EnvironmentMisc::collisionCheck(glm::vec3 collisionContourPoint)
{
...
bool EnvironmentMisc::collisionBetweenPoint(glm::vec3 point, float threshold, unsigned int triangle)
{
...
bool EnvironmentMisc::BarycentricCalculation(glm::vec3 point, float dist, unsigned int triangle)
{
