...
EnvironmentLightPoles::EnvironmentLightPoles() {}
EnvironmentLightPoles::~EnvironmentLightPoles() {}

void EnvironmentLightPoles::init()
{
	auto scene = aiImportFile("models/Environment_light_poles.OBJ", aiProcessPreset_TargetRealtime_MaxQuality);
	...
	if (scene && scene->mNumMeshes != 0)
	{
		...
		{
			printf("Loading Environment (Light Poles) Textures ... ");
...
void EnvironmentLightPoles::update()
{
	this->entity->getCarObject(car);
	...
	car.setCarCollisionRespondEnvLightPoles(collisionRespond);

	this->entity->setCarObject(car);
}

void EnvironmentLightPoles::calcBoundingBox(std::vector<std::vector<glm::vec3>> triangles, std::vector<glm::vec3>& boundingBox)
{
...
bool EnvironmentLightPoles::inBoundingBox(glm::vec3 point, std::vector<glm::vec3> boundingBox)
{
...
bool EnvironmentLightPoles::collisionCheck(glm::vec3 collisionContourPoint)
{
...
bool EnvironmentLightPoles::collisionBetweenPoint(glm::vec3 point, float threshold, unsigned int triangle)
{
...
bool EnvironmentLightPoles::BarycentricCalculation(glm::vec3 point, float dist, unsigned int triangle)
{
